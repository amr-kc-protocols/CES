...
-- ===== 1 of 3 — corrections ================================================
-- Applies all ${CORRECTED.length} and reports on every one. Expect ${CORRECTED.length} x 'updated' on a
-- first run, ${CORRECTED.length} x 'already applied' on a second. Any 'NOT FOUND' row means the
...
const rows = CORRECTED.map((f, i) => {
  // The first row carries the casts that fix the column types for the whole
  // VALUES list; without them Postgres has to guess, and guesses unknown.
  const cast = i === 0 ? ['::int', '::text', '::text', '::text[]', '::int'] : ['', '', '', '', '']
  return `    -- ${f.id}: ${f.why}\n    (${f.id}${cast[0]}, ${q(f.match)}${cast[1]}, ${q(f.stem ?? f.match)}${cast[2]}, ${arr(f.options)}${cast[3]}, ${f.answer}${cast[4]})`
})
out.push(rows.join(',\n'))
...
-- ===== 2 of 3 — retirements ================================================
-- ${RETIRED.length} items whose clinical content a candidate could argue. Set inactive, never
...
out.push(
  RETIRED.map(
    (r, i) =>
      `    -- ${r.id}: ${r.why}\n    (${r.id}${i === 0 ? '::int' : ''}, ${q(r.match)}${i === 0 ? '::text' : ''})`,
  ).join(',\n'),
)
...
-- ===== 3 of 3 — verification ===============================================
-- Expect: 161 active / ${RETIRED.length} retired, GCS keyed 12, drip rate keyed 125 gtt/min.
