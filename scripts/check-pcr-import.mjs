...
// ----- medical necessity -----------------------------------------------------
//
// The screen has described itself as checking medical necessity since it was
// built, and nothing did. Elite does not either: a non-emergent Medicare
// hospital-to-SNF transfer fired no rule about the certification in live
// testing. The gate has to be tight — a necessity finding on a chart whose
// payer the export never stated is the same false positive as any other, on the
// most consequential question this tool asks.

function iftChart(over = {}) {
  const b = page(1)
    .field('EMS Agency', 'KS-EXAMPLE- Ground')
    .field('Incident #', over.run ?? '99000030')
    .field('Date of Service:', '08/16/2026 10:02:00')
    .field('Type of Service Requested', 'Interfacility Transfer')
    .field('Type of Encounter', 'Interfacility Transfer')
    .field('Unit Transport and Equipment Capability', over.capability ?? 'Ground Transport (BLS Equipped)')
    .field('Incident Address', 'EXAMPLE HOSPITAL')
    .field('Unit Disposition', 'Patient Contact Made')
    .field('Patient Evaluation/Care', 'Patient Evaluated and Care Provided')
    .field('Destination Name', 'EXAMPLE SKILLED NURSING')
    .field('Destination reason', 'Closest Facility')
    .field('Transport Disposition', 'Transport by This EMS Unit (This Crew Only)')
    .field('Transport Mode Descriptors', 'No Lights or Sirens')
    .field('Primary Method of Payment', over.payment ?? 'Insurance')
    .table('Insurance Company Name Insurance Group Name', over.insurer ?? 'MEDICARE PART B')
    .field('Reason for Interfacility Transfer/Medical Transport', over.iftReason ?? 'Continuation of Care')
  if (over.servicesUnavailable !== false) {
    b.field('List the services the patient requires that are not available at the sending facility',
      'Skilled nursing and wound care not available at the sending facility.')
  }
  if (over.mnc) {
  ...
  b.field('Narrative Patient Care Report Narrative', over.narrative ?? IFT_NARRATIVE)
    .table(
      'Vital Signs Vitals Date/Time HR SBP/DBP (MAP) Resp. Rate SpO2 Pain',
      '08/16/2026 10:10:00 || PTA = No Vance, Robin (10101) 72 128 / 76 ( 99 ) 16 Normal 0 Numeric '
        + '08/16/2026 10:35:00 || PTA = No Vance, Robin (10101) 74 126 / 78 ( 98 ) 16 Normal 0 Numeric',
    )
    .table('Crew Member Crew Member Response Role Crew Member ID Crew Member Level',
      'Primary Patient Caregiver-Transport Vance, Robin (10101) EMT')
    .field('Crew Member Completing this Report', 'Vance, Robin (10101)')
    .field('Signatures Type of Person Signing', 'Crew Member')
    .field('Printed Name', 'Robin Vance')
...
const IFT_NARRATIVE =
  'unit dispatched non-emergent for an interfacility transfer from the hospital to a skilled nursing '
  + 'facility. patient reports no pain. history of a wound requiring skilled nursing care. assessment '
  + 'unremarkable, a&ox4, skin warm and dry. patient moved to the cot by drawsheet, transported without '
  + 'incident and report given to the receiving nurse. no treatment required en route.'

{
  const chart = iftChart()
  check(m.needsMedicalNecessity(chart) === true,
    'a non-emergent Medicare interfacility transport needs a certification')
  const r = m.autoReview(chart)
  check(
    r.types.join() === 'cqm,necessity',
    'so the review gets the necessity block alongside the ordinary one, not instead of it',
    r.types.join(),
  )
  const asked = m.visibleQuestions(r.types, r.categories).map((q) => q.id)
  check(
    asked.includes('mnc.present') && asked.includes('dem.locations'),
    'and is asked both sets of questions',
  )
}

{
  // The gate is closed by any one part being absent or different.
  check(
    m.needsMedicalNecessity(iftChart({ run: '99000031', payment: 'Self Pay', insurer: 'NONE' })) === false,
    'a self-pay transfer does not need one',
  )
  const emergent = iftChart({ run: '99000032' })
  check(
    m.needsMedicalNecessity({ ...emergent, transportDescriptors: 'Lights and Sirens', transportMode: undefined }) === false,
    'an emergent transport does not need one',
  )
  check(
    m.needsMedicalNecessity({ ...emergent, encounterType: undefined, serviceRequested: undefined, natureOfCall: undefined, iftReason: undefined }) === false,
    'and neither does a 911 call',
  )
}

{
  // The check that can be made with certainty: a certification that is present
  // and says the patient could have gone by van.
  const r = m.autoReview(iftChart({ run: '99000033', mnc: { carOrVan: 'Yes' } }))
  check(r.answers['mnc.notCarOrVan'] === false, 'a certification answering Yes to the van question is a finding')
  const f = r.flags.find((x) => x.title.includes('defeats its own claim'))
  check(f?.severity === 'stop', 'and it stops the chart', JSON.stringify(r.flags.map((x) => x.title)))
}

{
  // Bed confined on the certification, ambulatory in the narrative.
  const r = m.autoReview(
    iftChart({
      run: '99000034',
      mnc: { bedConfined: 'Yes', carOrVan: 'No' },
      narrative: IFT_NARRATIVE.replace('patient moved to the cot by drawsheet', 'patient ambulated to the cot unassisted'),
    }),
  )
  check(r.answers['mnc.consistent'] === false, 'a bed-confined claim against an ambulatory narrative is a finding',
    JSON.stringify(r.sources['mnc.consistent']))
  check(
    r.flags.some((x) => x.severity === 'stop' && x.title.includes('Bed confined')),
...
{
  // Convenience is an honest answer that also documents non-coverage.
  const r = m.autoReview(iftChart({ run: '99000036', iftReason: 'Convenience Transfer (Patient Request)' }))
  check(
    r.flags.some((x) => x.title.includes('convenience')),
    'a convenience transfer is flagged before the claim goes out',
    JSON.stringify(r.flags.map((x) => x.title)),
  )
}
