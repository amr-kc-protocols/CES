...
import type { AcademyCohort, AcademyDay, SessionArrangement, Trainee } from '../../types'
...
/**
 * Trainee-facing Phase 2 schedule: an academy arc map (advance organizer) plus
 * each session's computed timeline, education-time total, and Field Guide links.
 * Academy completion is an internal record only — no CE language.
 */
export function phase2ScheduleHTML(cohort: AcademyCohort, arrangements: Record<string, SessionArrangement>): string {
  ...
  const arc = `
    <h2>Academy arc — the 5 clinical sessions</h2>
    <table><tr><th class="num">#</th><th>Session</th><th>Focus</th></tr>
      ${t.sessions
        .map(
          (s) =>
            `<tr><td class="num">${s.order}</td><td><strong>${esc(s.title)}</strong>${s.mode === 'at-home' ? ' <span class="badge">At home</span>' : ''}</td><td>${esc(s.objectives[0] ?? '')}</td></tr>`,
        )
        .join('')}
    </table>`
  ...
  return `
    <h1>${esc(cohort.label)} — Phase 2 (Clinical)</h1>
    <p class="sub">${esc(t.phase.name)} · internal academy record (not CE)</p>
    ${arc}
    ${sessionsHtml}
    <p class="footer">Generated by AMR CES · ${esc(formatDate())} · Field Guide links open the teaching content; CES orchestrates, the Field Guide delivers.</p>`
}
